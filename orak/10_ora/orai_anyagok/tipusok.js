...
*/
function elsoNForLooppal1(input) {
    var osszeg = 0;
...
function elsoNForLooppal2(input) {
    var osszeg = 0;
    for (var i = 0; i < input + 1; i++) {
        osszeg = osszeg + i;
        // console.log(osszeg);
...
*/
function haAkkorKlasszikusan(allitasErtek1, allitasErtek2) {
    var output = (!allitasErtek1 || allitasErtek2);
    {
        return output;
    }
    ;
...
*/
function tagadasKlasszikusan(allitasErtek) {
    var output = (!allitasErtek);
    {
        return output;
    }
    ;
...
*/
function haAkkorTagadasa1(allitasErtek1, allitasErtek2) {
    var output = (tagadasKlasszikusan(haAkkorKlasszikusan(allitasErtek1, allitasErtek2)));
    {
        return output;
    }
    ;
}
function haAkkorTagadasa2(allitasErtek1, allitasErtek2) {
    var output = (allitasErtek1 && !allitasErtek2);
    {
        return output;
    }
...
function sokVagy(l) {
    var total = false;
    for (var i = 0, n = l.length; i < n + 1; ++i) {
        console.log(total);
        total = total || l[i];
    }
    {
...
*/
function nFrodo(hossz) {
    var output = [];
    for (var i = 0, n = hossz; i < n + 1; ++i) {
        console.log(output);
...
*/
function pluszBaggins(input) {
    var output = input.concat(" Baggins");
    {
        return output;
    }
}
...
*/
var nevek = ["Frodo", "Bilbo", "Sam"];
function nRandomNevek(hossz) {
    var output = [];
    for (var i = 0, n = hossz; i < n + 1; ++i) {
...
var hobbitNevek = [{ neve: "Frodo", kora: 50 }, { neve: "Bilbo", kora: 111 }, { neve: "Sam", kora: 39 }];
/*

Ezzel ugyanaz, mint fent

*/
function nRandomNevek2(hossz) {
    var output = [];
    for (var i = 0, n = hossz; i < n + 1; ++i) {
        var r = Math.floor(Math.random() * 3);
        console.log(output);
